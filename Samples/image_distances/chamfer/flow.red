...
loadImage: does [
	...
	if not none? tmp [
		...
		binaryMat: matrix/init 2 bitSize imgSize
		flowMat: matrix/init 2 bitSize imgSize
...
view win: layout [
	...
	canvas3: base 20x256 
	return
	text 110 "Distance" middle
	sl0: slider 85x25 [
		if isFile [
			distance: 0.05 + to-float (face/data * 49.95)
			fnz/text: form round/to (distance) 0.01
			computeFlow 
		]
	]
	fnz: field 40 "10.0"
	
	text 120 "Gradient Threshold" middle
	sl1: slider 75x25 [
		if isFile [
			threshold: 1 + (to-integer face/data * 99)
			fgt/text: form round/to (threshold) 0.01
			computeFlow 
		]
	]
	fgt: field 30 "0" 

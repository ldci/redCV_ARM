...
win: layout [
	...
	text 55 "Frequency" middle
	sl1: slider 140x25 [f: round/to to-float face/data 0.01 ffreq/text: form f
					showFilter if auto?[filterImage]]
	ffreq: field 40
	
	text 55 "Angle" middle 
	sl2: slider 140x25 [angle: to-integer face/data * 180 
	...
	return
	text 55 "X Sigma" middle
	sl3: slider 140x25 [sigma_x: 0.5 + to-float face/data * 18.5
					fx_sigma/text: form round/to sigma_x 0.1 
					showFilter if auto?[filterImage]]
	fx_sigma: field 40 
	
	
	text 55 "Y Sigma" middle
	sl4: slider 140x25 [sigma_y: 0.5 + to-float face/data * 18.5
					fy_sigma/text: form round/to sigma_y 0.1
					showFilter if auto?[filterImage]]
	fy_sigma: field 40 
	
	
	text 55 "Radius" middle
	sl5: slider 140x25 [radius: 1 + to-integer face/data * 49 fradius/text: form radius
					showFilter if auto?[filterImage]]
	fradius: field 40 

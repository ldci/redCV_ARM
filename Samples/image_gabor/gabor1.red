...
win: layout [
	...
	text 70 "Frequency" 
	sl1: slider 125x25 [f: round/to to-float face/data 0.01 ffreq/text: form f
					showFilter]
	ffreq: field 40
	return
	
	text 70 "Angle" 
	sl2: slider 125x25 [angle: to-integer face/data * 180
	...
	return
	text 70 "X Sigma" 
	sl3: slider 125x25 [sigma_x: 1.0 + round to-float face/data * 99
					fx_sigma/text: form round sigma_x showFilter]
	fx_sigma: field 40 
	
	return
	text 70 "Y Sigma" 
	sl4: slider 125x25 [sigma_y: 1.0 + round to-float face/data * 99
					fy_sigma/text: form round sigma_y showFilter]
	fy_sigma: field 40 
	
	return
	text 70 "Radius" 
	sl5: slider 125x25 [radius: 1 + to-integer face/data * 99 fradius/text: form radius
					showFilter]
	fradius: field 40 
	return

...
"Makes Hough Space transform"
	...
	while [y < h] [
		...
		while [x < w][
			xf: (as float! x) - cx
			idx: ((y * w) + x) * unit1
			i: vector/get-value-int as int-ptr! (matValue + idx) unit1
			...
			if i > treshold [ 
				...
				while [t < maxTheta] [
					...
					if (idx2 >= accValue)  and (idx2 < accTail) [
						ii: vector/get-value-int as int-ptr! idx2 unit2 
						;rcvSetIntValue as integer! idx2 ii + 1 unit2
						p4: as int-ptr! idx2
						p4/value: switch unit2 [
        					1 [ii + 1 and FFh or (p4/value and FFFFFF00h)]
        					2 [ii + 1 and FFFFh or (p4/value and FFFF0000h)]
        					4 [ii + 1]
    					]

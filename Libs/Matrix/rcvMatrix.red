...
rcvGetInt2D: routine [
    ...
    idx: (y * width + x) * unit    ;-- 0-based byte offset
...
"Get float matrix value"
	...
	idx: (y * width + x) * unit    ;-- 0-based byte offset
    vector/get-value-float (mvalue + idx) unit
] 
...
"Set float matrix value"
	...
	unit		[integer!]
	idx			[integer!]
	p           [byte-ptr!]  ; <-- ajout d'un pointeur intermédiaire
][
	vec: mat/get-data mObj
	unit: mat/get-unit mObj
	width: mat/get-cols mObj
	...
	;idx: (idx - 1) * unit			;--red/S 0-based index
	idx: (y * width + x) * unit    ;-- 0-based byte offset
	p: mvalue + idx             ; <-- ON AJOUTE L'OFFSET ICI !!
	either unit = 8 [
		pt64: as float-ptr! p
		pt64/value: val]
		[pt32: as float32-ptr! p
		pt32/value: as float32! val]
]
...
"Red Image to integer or char [0..255] 2-D Matrix "
    ...
    ;w: image/get-width src 
    ;h: image/get-height src 
    ...
    while [y < h] [
    	x: 0
       	while [x < w][
        	...
        	;rgb: r + g + b / 3
        	p4: as int-ptr! (dValue) ;-- On caste l'adresse exacte en int-ptr!
    		p4/value: switch unit [
        		1 [rgb and FFh or (p4/value and FFFFFF00h)]
        		2 [rgb and FFFFh or (p4/value and FFFF0000h)]
        		4 [rgb]
    		]
           	x: x + 1
           	pix1: pix1 + 1
...
"Matrix to Red Image"
	...
	handle/value: as int-ptr! 0
    pixD: image/acquire-buffer dst as ptr-ptr! :handle
    w: IMAGE_WIDTH(dst/size) 
    h: IMAGE_HEIGHT(dst/size) 
    ;w: image/get-width dst 
    ;h: image/get-height dst 
    vec: mat/get-data mObj
	unit: mat/get-unit mObj
    ...
    image/release-buffer dst handle/value yes
]
...
"Matrix to Red Image"
    ...
    while [y < h] [
    	x: 0
       	while [x < w][
       		pixD/value: vector/get-value-int as int-ptr! value unit
       		value: value + unit
           	pixD: pixD + 1
           	x: x + 1
       ]
...
"Splits an image to 4 8-bit vectors"
	src			[image!]
	vec0		[vector!]
	vec1		[vector!]
	vec2		[vector!]
    ...
    while [y < h] [
    	x: 0
       	while [x < w][
       		dvalue0/value: as byte! (pix1/value >>> 24)
			dvalue1/value: as byte! (pix1/value and 00FF0000h >> 16)
			dvalue2/value: as byte! (pix1/value and FF00h >> 8)
			dvalue3/value: as byte! (pix1/value and FFh)
           	x: x + 1
           	pix1: pix1 + 1
...
"Merge 4 8-bit matrices to image"
    ...
    while [y < h] [
        x: 0
        while [x < w][
            ; Lecture directe de l'octet, plus rapide que rcvGetIntValue
            a: as integer! value0/value
            r: as integer! value1/value
            g: as integer! value2/value
            b: as integer! value3/value

            ; Sécurité pour les limites 8-bit
...
"Red image to array"
	...
	*Mat	[int-ptr!]
    ...
    ;h: image/get-height src 
    handle/value: as int-ptr! 0
	*Mat: image/acquire-buffer src as ptr-ptr! :handle
	...
	image/release-buffer src handle/value no
	blk
]
...
"Red image to a block of blocks of float values"
	...
	*Mat idx	[int-ptr!]
	...
	sint64!: alias struct! [int1 [integer!] int2 [integer!]]
	w: IMAGE_WIDTH(src/size)
    h: IMAGE_HEIGHT(src/size)
    ;w: image/get-width src 
    ;h: image/get-height src 
...
;--old routines to be improved but called by other libs. Unsafe!!!
